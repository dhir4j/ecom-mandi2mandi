...
function ProductsContentEnhancedInner({ initialProducts, categories }: ProductsContentProps) {
  ...
  const clearAll = () => {
    setSearchQuery('');
    // This will trigger a re-render of ProductFilters, which will then clear its own state.
    // A better way would be to lift the state up, but for now we can reload the filters component.
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Showing {finalProducts.length} of {initialProducts.length} products
          </p>
        </div>
          ...
          <Sheet>
            ...
            <SheetContent side="left" className="w-80 overflow-y-auto p-0">
              ...
              <div className="p-6">
               ...
               <SheetClose className="w-full rounded-none" asChild>
                  <Button className="w-full rounded-none">Show Results</Button>
               </SheetClose>
            </SheetContent>
          </Sheet>
        </div>
    ...
    <Suspense fallback={
      <main className="flex-grow container mx-auto py-8 px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <Skeleton className="hidden lg:block h-[600px] lg:col-span-1" />
